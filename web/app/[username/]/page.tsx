...
export default async function ProfilePage({ params }: { params: { username: string } }) {
    ...
    return (
        <div className="min-h-screen bg-[#080E1D] text-white selection:bg-indigo-500/30">
            ...
            <main className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
                    ...
                    <div className="relative h-48 md:h-64 w-full bg-slate-800">
                        <Image
                            ...
                            priority
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-[#080E1D]/80 to-transparent" />
                    </div>
                    ...
                    <div className="relative px-6 md:px-12 pb-10">
                        ...
                        <div className="relative -mt-20 md:-mt-24 mb-4 md:mb-6 flex justify-between items-end">
                            <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-[#080E1D] overflow-hidden bg-slate-800 shadow-xl">
                                <Image
                                    src={profile.avatar}
                                    alt={profile.name}
                                    fill
                        ...
                        <div className="mb-8">
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {profile.isPremium && (
                                    <div className="flex items-center gap-1 bg-gradient-to-r from-amber-200 to-yellow-400 text-yellow-900 text-xs font-bold px-2 py-1 rounded-full">
                            ...
                            <div className="flex flex-wrap items-center gap-4 mt-4 text-slate-300">
                                {(profile.jobTitle || profile.company) && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span>{profile.jobTitle} {profile.company && `at ${profile.company}`}</span>
                                    </div>
                                )}
                                {profile.city && (
                                    <div className="flex items-center gap-2">
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {profile.lookingFor.map((item: string) => (
                                                <span key={item} className="bg-white/5 border border-white/10 text-white px-3 py-1.5 rounded-lg text-sm">
                                                    {item}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                {profile.interests && profile.interests.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {profile.interests.map((interest: string) => (
                                            <span
                                                key={interest}
                                                className="bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-3 py-1.5 rounded-full text-sm font-medium"
                                            >
                                                {interest}
                                            </span>
                                        ))}
                                    ...
                                    <p className="text-slate-400 italic">No interests added yet.</p>
                                )}
                            </div>
                        </div>
                ...
                <div className="sm:hidden fixed bottom-0 left-0 w-full p-4 bg-[#080E1D]/80 backdrop-blur-xl border-t border-white/10 z-50 rounded-t-3xl">
                    <Link href="/" className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-indigo-500 to-fuchsia-600 text-white px-6 py-4 rounded-2xl font-bold shadow-lg shadow-indigo-500/25">
